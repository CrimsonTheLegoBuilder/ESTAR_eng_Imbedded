...
실시간성을 띄게 프로그래밍을 하는 건 꽤 오래 시간을 잡아먹고 안정적으로 동작하지도 않는다.
다만 위에 있는 모터는 한 번만 각도를 조절하고 나면 움직일 필요가 없으므로 움직이지 않아도 된다.
시범 운행까지는 아직 시간이 남긴 했는데...

그냥 폴링 방식으로 새로 구현을 하도록 한다. ESP32도 추가로 그냥 여러개를 주문하도록 해보자.
ESP32의 역할을 아주 축소한다. 회전체 모터와 펌프를 제어하는 컨트롤러를 따로 두도록 만드는 건 어떤가? 
